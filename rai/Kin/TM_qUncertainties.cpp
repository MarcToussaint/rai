...
void TM_qUncertainties::phi(arr& q, arr& J, const mlr::KinematicWorld& G, int t) {
  uint n=dim_phi(G);
...
uint TM_qUncertainties::dim_phi(const mlr::KinematicWorld& G) {
  uint n=0;
  for(mlr::Joint *j : G.fwdActiveJoints) if(j->uncertainty) n += j->dim;
  return n;
}

mlr::String TM_qUncertainties::shortTag(const mlr::KinematicWorld& G){
