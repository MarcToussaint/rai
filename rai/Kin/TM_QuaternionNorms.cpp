...
void TM_QuaternionNorms::phi(arr &y, arr &J, const mlr::KinematicWorld &G, int t){
    uint n=dim_phi(G);
    y.resize(n);
    if(&J) J.resize(n, G.q.N).setZero();
    uint i=0;
    for(const mlr::Joint *j: G.fwdActiveJoints) if(j->type==mlr::JT_quatBall || j->type==mlr::JT_free || j->type==mlr::JT_XBall){
        arr q;
...
uint TM_QuaternionNorms::dim_phi(const mlr::KinematicWorld &G){
    uint i=0;
    for(const mlr::Joint* j:G.fwdActiveJoints){
