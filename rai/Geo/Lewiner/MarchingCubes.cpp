 ...
 * @date    01/21/2009
 *
 * @brief   MarchingCubes Algorithm, with corrections of many people
 */
...
{
  if( !_ext_data )
    _data    = new MCreal [_size_x * _size_y * _size_z] ;
...
void MarchingCubes::compute_intersection_points( MCreal iso )
//-----------------------------------------------------------------------------
{
...
//-----------------------------------------------------------------------------
{
  MCreal t, At=0, Bt=0, Ct=0, Dt=0, a, b ;
  char  test =  0 ;
...
MCreal MarchingCubes::get_x_grad( const int i, const int j, const int k ) const
//-----------------------------------------------------------------------------
{
  if( i > 0 )
  {
...
MCreal MarchingCubes::get_y_grad( const int i, const int j, const int k ) const
//-----------------------------------------------------------------------------
{
  if( j > 0 )
  {
...
MCreal MarchingCubes::get_z_grad( const int i, const int j, const int k ) const
//-----------------------------------------------------------------------------
{
  if( k > 0 )
  {
...
//-----------------------------------------------------------------------------
{
  ...
  MCreal u = ( _cube[0] ) / ( _cube[0] - _cube[1] ) ;
  ...
  u = (MCreal) sqrt( vert->nx * vert->nx + vert->ny * vert->ny +vert->nz * vert->nz ) ;
  if( u > 0 )
  {
    vert->nx /= u ;
    vert->ny /= u ;
...
//-----------------------------------------------------------------------------
{
  ...
  MCreal u = ( _cube[0] ) / ( _cube[0] - _cube[3] ) ;
...
//-----------------------------------------------------------------------------
{
  ...
  MCreal u = ( _cube[0] ) / ( _cube[0] - _cube[4] ) ;
