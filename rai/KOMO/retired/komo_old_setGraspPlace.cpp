...
//  rai::String& endeffRef = world.getShapeByName(graspRef)->body->inLinks.first()->from->shapes.first()->name;
  ...
  setTask(time-.1, time, new TM_Default(TMT_vec, world, endeffRef, Vector_z), OT_sumOfSqr, {0.,0.,1.}, 1e3);
  setTask(time-.1, time, new TM_Default(TMT_posDiff, world, graspRef, NoVector, object, NoVector), OT_sumOfSqr, NoArr, 1e3);
  setTask(time-.1, time, new TM_Default(TMT_quatDiff, world, graspRef, NoVector, object, NoVector), OT_sumOfSqr, NoArr, 1e3);
...
//#    (MakeJoint rigidZero Hand Obj){ time=1 }
  ...
  if(stepsPerPhase>2){ //otherwise: no velocities
    setTask(time-.2, time-.05, new TM_Default(TMT_pos, world, object), OT_sumOfSqr, {0.,0.,-.1}, 1e1, 1); //move down
    setTask(time, time+.15, new TM_Default(TMT_pos, world, object), OT_sumOfSqr, {0.,0.,.1}, 1e1, 1); // move up
...
void KOMO::setPlace(double time, const char* endeffRef, const char* object, const char* placeRef){
  ...
  if(stepsPerPhase>2){ //otherwise: no velocities
    setTask(time-.15, time, new TM_Default(TMT_pos, world, object), OT_sumOfSqr, {0.,0.,-.1}, 1e1, 1);
    setTask(time, time+.15, new TM_Default(TMT_pos, world, object), OT_sumOfSqr, {0.,0.,.1}, 1e1, 1); // move up
  }

  setTask(time-.02, time, new TM_Default(TMT_posDiff, world, object, NoVector, placeRef, NoVector), OT_sumOfSqr, {0.,0.,.1}, 1e3);
