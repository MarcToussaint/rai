...
struct PrimalDualProblem : ScalarFunction {
  ...
  PrimalDualProblem(const arr& x, const shared_ptr<MathematicalProgram>& P, rai::OptOptions opt=NOOPT, arr& lambdaInit=NoArr);
...
struct OptPrimalDual {
  ...
  OptPrimalDual(arr& x, arr& dual, const shared_ptr<MathematicalProgram>& P, int verbose=-1, rai::OptOptions opt=NOOPT);
  ~OptPrimalDual();
