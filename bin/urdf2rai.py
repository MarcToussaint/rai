...
for link in links:
    ...
    for visual in link.findall('visual'):
        print('visual %s_1 (%s) {  ' % (name, name))
        writeShape(visual)
        print('visual\n}\n') # end of shape

    # collision shape
    for collision in link.findall('collision'):
        print('collision %s_0 (%s) {  ' % (name, name))
        print('color:[.8 .2 .2 .5]')
    ...
    if joint.find('child') is not None:
        ...
        if elem is not None:
            ...
            if lo is not None:
                print('limits:[%s %s]' % (lo, up))
            if vel is not None:
                print('ctrl_limits:[%s -1 %s]' % (vel, eff)) #the 2nd value is an acceleration limit
