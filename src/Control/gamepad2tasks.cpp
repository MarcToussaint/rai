...
Gamepad2Tasks::Gamepad2Tasks(TaskControlMethods& _TC, const mlr::KinematicWorld& K, const arr& _q0)
  ...
  if(true || mlr::getParameter<bool>("oldfashinedTaskControl", true)){
    ...
    if(robot=="pr2") headAxes = new CtrlTask("endeffHead", new TaskMap_qItself(K, "head_pan_joint", "head_tilt_joint"), .5, 1., 1., 1.);
    if(robot=="baxter") headAxes = new CtrlTask("endeffHead", new TaskMap_qItself(K, "head_pan"), .5, 1., 1., 1.);
    ...
    if(robot=="pr2") {
      base = new CtrlTask("endeffBase", new TaskMap_qItself(K, "worldTranslationRotation"), .2, .8, 1., 1.);
      torso = new CtrlTask("torso_lift_link", new TaskMap_Default(posTMT, K, "torso_lift_link_0"), .2, .8, 1., 1.);
      gripperL = new CtrlTask("gripperL", new TaskMap_qItself(K.getJointByName("l_gripper_joint")->qIndex, K.getJointStateDimension()), 2., .8, 1., 1.);
      gripperR = new CtrlTask("gripperR", new TaskMap_qItself(K.getJointByName("r_gripper_joint")->qIndex, K.getJointStateDimension()), 2., .8, 1., 1.);
    }
    if(robot=="baxter") {
      gripperL = new CtrlTask("gripperL", new TaskMap_qItself(K.getJointByName("l_gripper_l_finger_joint")->qIndex, K.getJointStateDimension()), 2., .8, 1., 1.);
      gripperR = new CtrlTask("gripperR", new TaskMap_qItself(K.getJointByName("r_gripper_l_finger_joint")->qIndex, K.getJointStateDimension()), 2., .8, 1., 1.);
    }
  }else{
...
//    homing->PD().setGains(10., 2.);
    ...
    headAxes = new CtrlTask("endeffHead", new TaskMap_qItself(K, "head_pan_joint", "head_tilt_joint"), .5, 1., 1., 1.);
    limits = new CtrlTask("limits", new TaskMap_qLimits(), .2, .8, 1., 1.);
    coll = new CtrlTask("collisions", new TaskMap_Proxy(allPTMT, {0u}, .1), .2, .8, 1., 1.);
    gripperL = new CtrlTask("gripperL", new TaskMap_qItself(K.getJointByName("l_gripper_joint")->qIndex, K.getJointStateDimension()), 2., .8, 1., 1.);
    gripperR = new CtrlTask("gripperR", new TaskMap_qItself(K.getJointByName("r_gripper_joint")->qIndex, K.getJointStateDimension()), 2., .8, 1., 1.);
