...
void GravityCompensation::learnGCModel() {
  ...
  for(rai::String joint : joints) {
    ...
    double c;
    betasGC[joint] = cv.calculateBetaWithCV(featuresGC(q, qSign, joint), u.sub({0,0},{ index, index+1}), lambdas, false, c);
    cout << c << endl;
  }
}
...
void GravityCompensation::learnFTModel() {
  ...
  arr mR = zeros(6);
  for(uint i = 0; i < 6; i++) {
    betaFTL.append(~cv.calculateBetaWithCV(PhiL, fL.sub({0,0},{ i, i+1}), lambdas, false, mL(i)));
    betaFTR.append(~cv.calculateBetaWithCV(PhiR, fR.sub({0,0},{ i, i+1}), lambdas, false, mR(i)));
  }
  betaFTL = ~betaFTL;
  betaFTR = ~betaFTR;
...
arr GravityCompensation::featuresGC(arr q, arr qSign, const rai::String& joint) {
  ...
  if(stictionFeature) Phi = catCol(Phi, sign(qSign.sub({0,0},{ index, index+1})));
...
void GravityCompensation::generatePredictionsOnDataSet(const arr& Q, const arr& U, const StringA& joints) {
  ...
  for(rai::String s : joints) {
    ...
    FILE(STRING(s << "_u.dat")) << U.sub({0,0},{ index, index+1});
  }
}
