...
arr Feature::phi_finiteDifferenceReduce(const FrameL& F) {
  ...
  // arr y0 = phi(F({0, -2}));
  // arr y1 = phi(F({1, -1}));
  arr y0 = phi(F({0, 0+order+1}));
  arr y1 = phi(F({1, 1+order+1}));
  order++;
  timeIntegral++;
...
VectorFunction Feature::asFct(const FrameL& F) { ///< direct conversion to vector function: use to check gradient or evaluate
