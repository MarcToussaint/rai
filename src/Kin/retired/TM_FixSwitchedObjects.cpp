...
//    }
    ...
    if(order==1) { //absolute velocities
      ...
      pos.order=1;
      pos.Feature::eval(y({M*i, M*i+2+1})(), (!!J?J({M*i, M*i+2+1})():NoArr), Ktuple);
      ...
      quat.order=1;
      quat.Feature::eval(y({M*i+3, M*i+6+1})(), (!!J?J({M*i+3, M*i+6+1})():NoArr), Ktuple);
#else
      ...
      TM_AngVel rot(id);
      rot.order=1;
      rot.phi(y({M*i+3, M*i+5+1})(), (!!J?J({M*i+3, M*i+5+1})():NoArr), Ktuple);
#endif
      ...
      pose.order=2;
      pose.Feature::eval(y({M*i, M*i+6+1})(), (!!J?J({M*i, M*i+6+1})():NoArr), Ktuple);
#else
      TM_Default pos(TMT_pos, id);
      pos.order=2;
      pos.Feature::eval(y({M*i, M*i+2+1})(), (!!J?J({M*i, M*i+2+1})():NoArr), Ktuple);
