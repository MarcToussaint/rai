...
//    for(uint i=0;i<G.N;i++) cout <<i <<' ' <<G(i)->joints.N <<' ' <<G(i)->q.N <<' ' <<G(i)->getJointStateDimension() <<endl;
    rai::Array<rai::Joint*> matchingJoints = getMatchingJoints(G.sub({-1-order,0}), effectiveJointsOnly);
    uint ydim=0;
...
//        J[k] += y(k) * (0.5*hj/tau) * Jtau;
    ...
    rai::Array<rai::Joint*> matchingJoints = getMatchingJoints(Ktuple.sub({-1-order,0}), effectiveJointsOnly);
