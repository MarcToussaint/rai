...
ConfigurationViewer& ConfigurationViewer::updateConfiguration(const Configuration& C, const FrameL& timeSlices, bool forceCopyMeshes) {
  ...
  if(C.proxies.N) {
    ...
    for(const Proxy& p: C.proxies){
      ...
      if(!p.collision){
        addDistMarker(C.frames.elem(p.A)->getPosition(), C.frames.elem(p.B)->getPosition(), s, .1, {1.,1.,0.});
      }else if(p.d<.05){
...
void ConfigurationViewer::setCamera(Frame* camFrame) {
  ...
  uint W=gl->width, H=gl->height;
  {
    ...
    if(camFrame){
      gl->camera = camFrame->getCameraFromAts();
    }else{
      gl->camera.setDefault();
    }
  }
...
void ConfigurationViewer::focus(const arr& position, double heightAbs){
  ...
  arr pos = position;
  cam.focus(pos, true);
  double dist = heightAbs * cam.fxycxy(1);
...
int ConfigurationViewer::view(bool pause, const char* _text, bool offscreen) {
  if(_text) text = _text;
  if(pause && (!text.N || text(-1)!=']')) text <<"\n[press key to continue]";

  ensure_gl();
  if(offscreen){ gl->renderInBack(); return 0; }
  if(pause) gl->raiseWindow();
  return gl->update(pause, nonThreaded);
}
