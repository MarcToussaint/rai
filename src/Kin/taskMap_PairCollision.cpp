...
void TaskMap_PairCollision::phi(arr& y, arr& J, const mlr::KinematicWorld& K, int t){
...
mlr::String TaskMap_PairCollision::shortTag(const mlr::KinematicWorld &G){
  return STRING("TaskMap_PairCollision"<<(i<0?"WORLD":G.frames(i)->name) <<':' <<(j<0?"WORLD":G.frames(j)->name));
}
