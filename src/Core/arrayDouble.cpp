...
arr finiteDifference_gradient(ScalarFunction f, const arr& x0, double y0, double eps){
  arr x, g(x0.N);
  double y;
...
arr finiteDifference_jacobian(VectorFunction f, const arr& x0, const arr& y0, double eps) {
  arr x, y, J(y0.N, x0.N);
  for(uint i=0; i<x0.N; i++) {
    x = x0;
    x.elem(i) += eps;
...
// /// numeric (finite difference) computation of the gradient
// arr finiteDifferenceJacobian(VectorFunction f, const arr& _x, arr& Janalytic, double eps) {
//   arr x=_x;
//   arr y, dx, dy, J;
//   y = f(x);
//   if(!!Janalytic){
...
bool checkGradient(ScalarFunction f,
  ...
  arr JJ = finiteDifference_gradient(f, x, y);
  uint i;
...
bool checkJacobian(VectorFunction f,
  ...
  arr y = f(x);
  arr J = y.J_reset();
  if(isRowShifted(J) || isSparseMatrix(J)) J = unpack(J);
  arr JJ = finiteDifference_jacobian(f, x, y);
#if 1
  uint k;
