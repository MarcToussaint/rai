...
MT::FileToken::FileToken(const char* _filename):filename(_filename), os(NULL), is(NULL){
}
...
std::ofstream& MT::FileToken::getOs(){
  ...
  if(!os){
    ...
    os=new std::ofstream;
    os->open(name);
    log() <<"opening output file `" <<filename <<"'" <<std::endl;
