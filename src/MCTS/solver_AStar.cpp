...
AStar_Node::AStar_Node(MCTS_Environment& world)
  : world(world), parent(NULL), d(0), time(0.){
...
AStar_Node::AStar_Node(AStar_Node* parent, const MCTS_Environment::Handle& a)
  : world(parent->world), action(a), parent(parent), d(parent->d+1){
...
void AStar_Node::getGraph(Graph& G, Node* n) {
  ...
  graphIndex = n->index;
  n->keys.append(STRING("d:" <<d <<" t:" <<time <<' ' <<action));
...
AStar::AStar(MCTS_Environment& world){
  root = new AStar_Node(world);
  queue.add(0., root);
}
