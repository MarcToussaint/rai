...
/** isovalue alias */
typedef double MCreal; //or float??
...
typedef struct
{
  MCreal  x,  y,  z ;  /**< Vertex coordinates */
  MCreal nx, ny, nz ;  /**< Vertex normal */
} Vertex ;
...
public :
  ...
  inline Vertex   * vert( const int i ) const { if( i < 0  || i >= _nverts ) return ( Vertex *)nullptr ; return _vertices  + i ; }
  /** accesses a specific triangle of the generated mesh */
  inline Triangle * trig( const int i ) const { if( i < 0  || i >= _ntrigs ) return (Triangle*)nullptr ; return _triangles + i ; }
   ...
   */
  inline void set_ext_data  ( MCreal *data )
   ...
   */
  inline const MCreal get_data  ( const int i, const int j, const int k ) const { return _data[ i + j*_size_x + k*_size_x*_size_y] ; }
  /**
   ...
   * \param k height of the cube
   */
  inline void  set_data  ( const MCreal val, const int i, const int j, const int k ) { _data[ i + j*_size_x + k*_size_x*_size_y] = val ; }
...
public :
   ...
   * \param iso isovalue
   */
  void run( MCreal iso = (MCreal)0.0 ) ;
...
protected :
   ...
   * \param iso isovalue
   */
  void compute_intersection_points( MCreal iso ) ;
   ...
   * \param k height of the cube
   */
  MCreal get_x_grad( const int i, const int j, const int k ) const ;
  /**
   ...
   * \param k height of the cube
   */
  MCreal get_y_grad( const int i, const int j, const int k ) const ;
  /**
   ...
   * \param k height of the cube
   */
  MCreal get_z_grad( const int i, const int j, const int k ) const ;
...
protected :
  ...
  MCreal     *_data       ;  /**< implicit function values sampled on the grid */
  ...
  MCreal      _cube[8]    ;  /**< values of the implicit function on the active cube */
